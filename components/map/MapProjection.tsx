  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < ORBIT_STEPS; i++) {
        ...
        if (pos) orbitPoints.push(latLonToXY(pos.lat, pos.lon));
      }
      ...
      if (orbitPoints.length > 1) {
        ...
        for (let i = 1; i < orbitPoints.length; i++) {
          const [prevX, prevY] = orbitPoints[i - 1];
          const [x, y] = orbitPoints[i];

          const dx = Math.abs(x - prevX);

          // If jump is too large, we crossed the dateline -> start new segment
      ...
      if (satPos) {
        const [sx, sy] = latLonToXY(satPos.lat, satPos.lon);
